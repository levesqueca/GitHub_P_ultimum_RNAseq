...
Make a subset for OosporeConversion
```{r}
...
Get a subset for my own GLM analysis  OOspore conversion
```{r}
library(reshape2)

my_counts_GLM <- dcast(data = subset_count, Time + RNA_Rep  ~ SequenceID, value.var = "Read_Counts")
...
write.table(fit2_pvalues, paste(sharedPath,mainFolder,"/",analysisDir, "/", Exp, "Poisson_glm_CAZy_Anyone_or_each_diff_from_ctrl.csv", sep=""), append = FALSE, quote = TRUE, sep = ",", col.names = NA, row.names = TRUE)
