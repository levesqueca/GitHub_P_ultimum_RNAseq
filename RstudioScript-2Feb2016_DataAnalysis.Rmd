...
ooGenMetaPath <- paste(sharedPath, 
                       ...
                       sep = "")
ooConvMetaPath <- paste(sharedPathAn,
...
Get all data together for oosporogenesis and oospore conversion and add the counts for libraries representing the same treatments:
```{r}

sharedPath <- "/isilon/biodiversity/users/shared/Pythium_ultimum_RNAseq/"

metafile1 <- "Oosporogenesis_Final_metadataTable_afterProcessing.csv"
metadataAdapRem1 <- read.csv(paste(shared_path,metafile1, sep=""), stringsAsFactors=FALSE)
...
metafile2 <- "OosporeConversion_Final_metadataTable_afterProcessing.csv"
Conversion_path   <- "Oospore_Conversion_TimeCourse/HiSeq_Analyses2/"
metadataAdapRem2 <- read.csv(paste(shared_path,Conversion_path,metafile2, sep=""), stringsAsFactors=FALSE)
...
write.table(data.frame(countsTopHat), file = file.path(sharedPath, "counts_readDGE_TopHat.Oospore_genesis_conversion.annotated2.csv"),
            sep  = ",", 
            # sep = "\t", 
            row.names = TRUE, 
            col.names = NA, 
...
#drop it in metadata too
Metadata_2exp_reduced   <- Metadata_2exp[!Metadata_2exp$LibraryName %in% controlToDrop, ]

write.table(Metadata_2exp_reduced, file = file.path(sharedPath, "Metadata.Oospore_genesis_conversion_reduced.csv"),
            sep  = ",", 
            # sep = "\t", 
            row.names = TRUE, 
            col.names = NA, 
...
# Must do the same on columns of data frame of counts, does the sub command on those selected only to add a c
colnames(countsTopHatDf)[grep("T0-1_BC01|T0-1_BC02|T0-2_Lib1|T0-4_BC02", colnames(countsTopHatDf))] <-
   sub("^T0-","T0-c",colnames(countsTopHatDf)[grep("T0-1_BC01|T0-1_BC02|T0-2_Lib1|T0-4_BC02", colnames(countsTopHatDf))])
...
Normalization after removing rDNA data.  Best approach to compare with Levesque et al 2010.
```{r}
library(Biostrings)
library("edgeR")
library("reshape2")
...
LEV_2010_rpkm <- read.xlsx(paste(sharedPath,"References/Genome_Biology_paper_AdditionalDataFile3.xlsx",sep=""), sheet="S9", startRow = 2)
...
length(unique(T_G_conversion_table[,1]))
length(unique(T_G_conversion_table[,2]))

T_G_conversion_table$Locus[duplicated(T_G_conversion_table[,2])]
