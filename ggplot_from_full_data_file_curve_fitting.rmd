...
Secreted_CAZy <- read.table("data/dbCAN_SecretedPyuu_Final_subsetColumns.csv", 
                                  sep = ",", 
                                  header = TRUE, 
                                  as.is = FALSE, 
                                  stringsAsFactors = FALSE)
...
fit3 <- glm(sub_my_counts_GLM[,4:5] ~ Condition, family = quasipoisson, sub_my_counts_GLM)
...
my_counts <- dcast(data = subset_count, SequenceID ~ TRT, value.var = "Read_Counts")
...
group2 <- sub("_.*","",colnames(my_counts))


cds2 <- DGEList( my_counts , group = group2 )
...
de.cmn2 <- exactTest( cds2 , common.disp = TRUE , pair = c( "Control" , "Cholesterol" ) )
...
write.table(Fit_results, paste(sharedPath,mainFolder,"/EdgeR_glmQLFit_secreted_output.csv", sep=""), append = FALSE, quote = TRUE, sep = ",",col.names = TRUE)

fit <- glmFit(cds2,design)
lrt <- glmLRT(fit)
topTags(lrt)

Fit_results <- data.frame(lrt$coefficients, lrt$table)
                          
write.table(Fit_results, paste(sharedPath,mainFolder,"/EdgeR_glmFit_secreted_output.csv", sep=""), append = FALSE, quote = TRUE, sep = ",",col.names = TRUE)
...
ii <- 15
  ...
  for(ii in 4:(ncol(my_counts_NL)-1)) { 
    print("######################################################################################################")   
...
# start_cent <- log(my_counts_NL[which.max(my_counts_NL[,ii]), 2] +1)
 ...
 whichmax1 <- which.max(my_counts_NL[,ii])
 whichmax2  <- which.max(my_counts_NL[-whichmax1,ii])
 
 start_cent  <- (log(my_counts_NL[whichmax1, 2] +1) + log(my_counts_NL[whichmax2, 2] +1)) /2

   NL_fit <- nls(my_counts_NL[,ii] ~   exp(max*exp(-((sqrt_time - cent)^2) / (2 * StDev^2))), algorith = "port",
