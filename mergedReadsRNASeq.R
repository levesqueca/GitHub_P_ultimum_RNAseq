...
GenomeDir_path <- "/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads/GenomeDir/"
P <- "Parent"
Gff3_overhang <- 486 #needs to be max len -1, max length of R1 ~280?
...
cmd <- paste(bowtie2_build_path, " -f ", fa, 
             " /home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads/References","/",paste("Pyuu_ref", sep="_"), sep="")
cmd
system(cmd)
###########################################################################################################################
...
for(k in 1:nrow(MetadataRawPairs)) {
  cmd <- with(MetadataRawPairs, paste(SeqPrep_path, "-f", R1, "-r", R2, "-1", 
                                      ...
                                      "-A", FwdAdapter, "-B", RevAdapter, "-s", paste("AdapRem_Merged_",LibraryName,".fastq.gz", sep="")))
}
cmd
sapply(cmd, function(x) system(x))
...
############################################################################################################################
#Need to unzip the merged fastq.gz files after adapter removal:
adap_zip <- list.files(path = ".", pattern = "AdapRem_Merged_.*fastq.gz$")
adap_zip
...
############################################################################################################################
#Make a Metadata table called MetadataAdapRem that has the raw reads rows collapsed.
for(k in 1:nrow(Metadata)){
  Metadata$Adapters_Removed <- paste("AdapRem_Merged_",Metadata$LibraryName,".fastq", sep="")
}
MetadataAdapRemMerged <- dcast(data = Metadata, LibraryName + Condition + TimePoint + MiSeqReplicate ~Read_Direction, value.var
                         = "Adapters_Removed", FUN=c)
MetadataAdapRemMerged$ShortName <- paste(MetadataAdapRemMerged$Condition, MetadataAdapRemMerged$TimePoint, MetadataAdapRemMerged$MiSeqReplicate, sep=".")
#I changed it to specify MiSeq Replicate instead of RNA Replicate
############################################################################################################################
...
#log <- "processed_merged_log"
for(k in 1:nrow(MetadataAdapRemMerged)) {
  cmd = with(MetadataAdapRemMerged, paste(PrinSeq_path, " -fastq ", R1,  
                                    " -ns_max_n ", nmax, 
                                    " -trim_left ", trim_left, 
                                    ...
                                    " -out_good ", paste("Processed_merged",LibraryName, sep="_"), 
                                    " -out_bad ", out_bad, 
                                    " -verbose ",  
                                    " -min_len ", min_len,
                                    #" -trim_to_len ", trim_to_len,
                                    " -no_qual_header ", 
                                    " -log ", "/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads/PrinSeq_logs","/",paste("Processed_log",LibraryName, sep="_"),
                                    sep=""))
}
cmd
sapply(cmd, function(x) system(x))
list.files(path = "/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads/PrinSeq_logs/", pattern=glob2rx("Processed_log*"), full.names=T)
ProcessedLogs_PrinSeq <- list.files(path = "/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads/PrinSeq_logs/", pattern=glob2rx("Processed_log*"), full.names=T)
...
for(k in 1:nrow(Metadata)){
  Metadata$Processed_Fastq <- paste("Processed_merged_",Metadata$LibraryName,".fastq", sep="")
}
MetadataProcessed <- dcast(data = Metadata, LibraryName + Condition + TimePoint + MiSeqReplicate ~ Read_Direction, value.var
...
for(k in 1:nrow(MetadataProcessed)){
  cmd = with(MetadataProcessed, paste(PrinSeq_path,
                                      ...
                                      " -out_good ", out_bad,
                                      " -out_bad ", out_bad,
                                      " -graph_data ", " /home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads/PrinSeq_graph_data","/",paste(LibraryName,"processed.gd", sep="_"),
                                      sep=""))
}
cmd
sapply(cmd, function(x) system(x))
list.files(path = "/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads/PrinSeq_graph_data/", pattern=glob2rx("*_processed.gd"), full.names=T)
Processed_graphs_PrinSeq <- list.files(path = "/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads/PrinSeq_graph_data/", pattern=glob2rx("*_processed.gd"), full.names=T)
...
cmd = with(MetadataProcessed, paste(tophat2_path, " -G ", gff3,
                                    " -p 12 -o ", "/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads", "/",LibraryName,"/",LibraryName,"_TopHat",
                                    " /home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads/References","/",bowind, 
                                    " ", R1," ",
                                    sep=""))
cmd
#system(cmd)
...
cmd = with(MetadataProcessed, paste(samtools1_path, 
                                    " sort", 
                                    " -n ",
                                    "/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads","/",LibraryName,"/",LibraryName,"_TopHat","/","accepted_hits.bam",
                                    " /home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads","/",LibraryName,"/",LibraryName,"_TopHat","/",LibraryName,"_sn",
                                    sep=""))
cmd                                    
sapply(cmd, function(x) system(x))
...
cmd = with(MetadataProcessed, paste(samtools1_path, 
                                    " view ", 
                                    " -o ", 
                                    " /home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads","/",LibraryName,"/",LibraryName,"_TopHat","/",LibraryName,"_sn.sam",
                                    " /home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads","/",LibraryName,"/",LibraryName,"_TopHat","/",LibraryName,"_sn.bam",
                                    sep=""))
cmd
sapply(cmd, function(x) system(x))
...
cmd = with(MetadataProcessed, paste(samtools1_path, 
                                    " sort ", 
                                    " /home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads","/",LibraryName,"/",LibraryName,"_TopHat","/","accepted_hits.bam",
                                    " /home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads","/",LibraryName,"/",LibraryName,"_TopHat","/",LibraryName,"_s",
                                    sep=""))
cmd
sapply(cmd, function(x) system(x))
...
cmd = with(MetadataProcessed, paste(samtools1_path, 
                                    " index ", 
                                    " /home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads","/",LibraryName,"/",LibraryName,"_TopHat","/",LibraryName,"_s.bam",
                                    sep=""))
cmd
sapply(cmd, function(x) system(x))
...
MetadataProcessed$countf_TopHat = paste(MetadataProcessed$LibraryName, "TopHat2.count", sep="_")
gff3 <- "/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads/References/Pyuu_ref_no_mito.gff3"
stranded <- "no"
MINAQUAL <- 10
cmd = with(MetadataProcessed, paste(htseq_count_path, 
                                    ...
                                    gff3,
                                    " > ",
                                    "/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads", "/", LibraryName, "/", LibraryName,"_TopHat","/",MetadataProcessed$countf_TopHat,
                                    sep=""))
cmd
sapply(cmd, function(x) system(x))
...
counts_list_TopHat = paste("/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads","/", MetadataProcessed$LibraryName,"/", MetadataProcessed$LibraryName,"_TopHat","/",MetadataProcessed$countf_TopHat, sep="")

counts_list_TopHat
counts_TopHat = readDGE(counts_list_TopHat)$counts
counts_TopHat

results <- "31-Aug-2015-TopHat-edgeR"
dir.create(results)
...
cmd = with(MetadataProcessed, paste(STAR_path, 
                                    " --genomeDir ", GenomeDir_path,
                                    " --readFilesIn ", R1, " ",
                                    " --outFileNamePrefix ", "/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads", "/", LibraryName, "/", LibraryName,
...
cmd = with(MetadataProcessed, paste(samtools1_path, 
                                    " view ", 
                                    " -b ",
                                    " -o ", "/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads", "/", LibraryName, "/", LibraryName,"Aligned.out.bam",
                                    " /home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads", "/", LibraryName, "/", LibraryName,"Aligned.out.sam",
                                    sep=""))
cmd
sapply(cmd, function(x) system(x))
# Sort BAM for IGV
cmd = with(MetadataProcessed, paste(samtools1_path, 
                                    " sort ", 
                                    "/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads", "/", LibraryName, "/", LibraryName,"Aligned.out.bam",
                                    " /home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads", "/", LibraryName, "/", LibraryName,"_s",
                                    sep=""))
# Index BAM files for IGV
cmd
sapply(cmd, function(x) system(x))
cmd = with(MetadataProcessed, paste(samtools1_path, 
                                    " index ", 
                                    "/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads", "/", LibraryName, "/", LibraryName,"_s.bam",
                                    sep=""))
cmd
sapply(cmd, function(x) system(x))
...
cmd = with(MetadataProcessed, paste(htseq_count_path, 
                                    ...
                                    gff3,
                                    " > ",
                                    "/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads", "/", LibraryName, "/", MetadataProcessed$countf,
                                    sep=""))
cmd
sapply(cmd, function(x) system(x))
############################################################################################################################
library("edgeR")
#Identify the count files and read them into R using readDGE
counts_list = sapply(file.path("/home/AAFC-AAC/girouxem/RNASeq/RNASeqSingleReads", MetadataProcessed$LibraryName, sep=""), dir,pattern=".count$",full.names=T)
counts_list
...
write.table(counts,file=file.path(results,"counts_readDGE_STAR.annotated.txt"),sep="\t",row.names=T,col.names=T,quote=F) 
#### HTSEQ_COUNT RESULTS 
# 4. Filter weakly expressed and noninformative (e.g., non-aligned) features: 
...
d = calcNormFactors(d)
#Inspect the relationships between samples using a multidimensional scaling (MDS) plot, as shown in Figure 4:
results <- "20-Aug-2015-Star-edgeR"
dir.create(results)
